...
<body>
    ...
    <a href="movie/{{ $movieID }}" class="group">
            ...
            <span class="font-inter text-sm mt-2 line-clamp-2 group-hover:line-clamp-none">{{ $movieYear }}</span>
...
function loadMore() {
    $.ajax({
        url: `${baseURL}/discover/movie?page=${page}&sort_by=${sortBy}&api_key=${api_Key}&vote_count.gte=${minimalVoter}`,
        type: "get",
        beforeSend: function () {
    ...
    .done(function (response) {
        ...
        if (response.results) {
            ...
            response.results.forEach(item => {
                ...
                let movieImage = item.poster_path
